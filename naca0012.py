...
epochs = 3000
model_name = 'model-n5'
directory = 'models/naca0012/'
training_data = [0, 2, 4, 6, 8]
...
###############################################################################
phi_post = 6
x_post_min, x_post_max = -1.5, 3.5
y_post_min, y_post_max = -2.5, 2.5
...
###############################################################################
x_lim_min, x_lim_max = -1, 3
y_lim_min, y_lim_max = -2, 2
...
df = pd.read_csv(f'training_data/aoa{training_data[0]}.csv')
for i in range(1, len(training_data)):
    df0 = pd.read_csv(f'training_data/aoa{training_data[i]}.csv')
...
if trainMode:
    ...
    for epoch in range(epochs):
        ...
        if (epoch+1) % 100 == 0:  # 100 epochta bir kayıp yazdır
...
# Generate grid
sample = 201
x_unique = np.linspace(scale(x_post_min, 'x'), scale(x_post_max, 'x'), sample)
y_unique = np.linspace(scale(y_post_min, 'y'), scale(y_post_max, 'y'), sample)
x_grid, y_grid = np.meshgrid(x_unique, y_unique)
x_grid_flatten = x_grid.flatten()
y_grid_flatten = y_grid.flatten()

# Post koordinatları kullanarak tahmin 
phi_input = torch.tensor((np.ones(sample*sample)*scale(phi_post, 'phi')), dtype=torch.float32, device=device).view(-1, 1)
...
df_post = pd.read_csv(f'training_data/aoa{phi_post}.csv')
...
strm = plt.streamplot(x_grid, y_grid, z_original_u, z_original_v, density=2,
...
strm = plt.streamplot(x_grid, y_grid, z_predicted_u, z_predicted_v, density=2,
